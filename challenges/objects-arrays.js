...
for (let index = 0; index < graduates.length; index++) {
  const graduate = graduates[index];
...
for (let index = 0; index < graduates.length; index++) {
  const graduate = graduates[index];
  graduate.university.includes("Uni") ? uni.push(graduate.university) : null;
}
console.log(uni);
...
populationTotal = zooAnimals.reduce((accumulator, animal) => accumulator + animal.population, 0)
